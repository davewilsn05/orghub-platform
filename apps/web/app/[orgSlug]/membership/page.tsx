...
export default function MembershipPage() {
  ...
  const statusColor = (status: string) => {
    if (status === "active" || status === "trialing") return "#059669";
    if (status === "past_due") return "#d97706";
    return "#6b7280";
  };

  if (loading) {
    return <main style={{ padding: "2rem" }}><div style={{ color: "#9ca3af" }}>Loading…</div></main>;
  }

  return (
    <main style={{ padding: "2rem", maxWidth: "640px" }}>
      <h1 style={{ fontSize: "1.75rem", fontWeight: 800, marginBottom: "0.5rem" }}>Membership</h1>
      ...
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "1.5rem", marginBottom: "1.5rem" }}>
        <h2 style={{ fontSize: "0.9rem", fontWeight: 700, margin: "0 0 1rem" }}>Current status</h2>
        {duesPaidThrough ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.875rem" }}>
              <span style={{ color: "#6b7280" }}>Dues paid through</span>
              <span style={{ fontWeight: 700 }}>{new Date(duesPaidThrough + "T00:00:00").toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</span>
            </div>
            {sub && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.875rem" }}>
                  <span style={{ color: "#6b7280" }}>Subscription status</span>
                  <span style={{ fontWeight: 700, color: statusColor(sub.status) }}>
          ...
          <div style={{ color: "#9ca3af", fontSize: "0.875rem" }}>No active membership on file.</div>
        )}
      </div>

      {/* Actions */}
      {sub?.stripe_customer_id && (sub.status === "active" || sub.status === "trialing") ? (
        ...
        <div style={{ color: "#9ca3af", fontSize: "0.875rem" }}>No membership plans are currently available. Please contact your administrator.</div>
      )}
    </main>
  );
}
